...
- 선택 영역 복사: **Ctrl+C** — 시스템 클립보드에 배경 투명 PNG(다른 앱에 그림으로 붙여넣기) + 앱 내부 버퍼에 노드·관계. **Ctrl+V** 로 커서 위치에 붙여넣는다(다른 탭도 가능). 관계는 양 끝이 모두 선택에 포함된 것만 따라오고, 붙여넣은 노드는 새 id 를 받아 선택 상태가 된다. 클립보드 이미지가 막히면 PNG 파일로 저장한다.
