...
- 📋 노드 목차: 🏷 왼쪽 버튼. 타입을 상위→하위 순서로 묶어 그 아래 노드를 나열한다. 여러 타입을 겸하면 주 타입에만 한 번 나오고, 같은 타입 안의 부모-자식은 들여쓴다. 항목을 한 번 누르면 그 노드를 선택하고 화면 중앙으로 옮기며, 두 번 누르면 노드 상세까지 연다. 타입 이름을 누르면 그 묶음을 접는다(접은 상태는 이번 세션 동안만 기억). 이름·설명·소속으로 걸러볼 수 있다.
